      ...
      <div class="overview-info">
        ...
        <div class="info">
          <div class="name-wrapper">
            <div class="name">
              {{ user.isMc ? user.nickName : user.fullName }}
            </div>
      ...
      <Tabs value="0" @update:value="handleTabChange">
        ...
        <TabPanels>
            ...
            <div class="tab-content-wrapper">
              ...
              >
                ...
                <FormField
                  ...
                  v-slot="$field"
                >
                  <label for="hostingStyles" class="form-label"
                    >Phong cách dẫn</label
                  >
                  <MultiSelect
          ...
          <TabPanel value="1">
            <div
              v-if="editingMode == EditingMode.Update"
              class="update-image-wrapper"
            >
              <div class="header">
                <Button
                  icon="pi pi-arrow-left"
                  @click="cancelEditImages"
                  class="back-button"
                />
          ...
          <TabPanel value="2">
            <div
              v-if="editingMode == EditingMode.Update"
              class="update-video-wrapper"
            >
              <div class="header">
                <Button
                  icon="pi pi-arrow-left"
                  @click="cancelEditVideos"
                  class="back-button"
                />
              ...
              >
                <Card>
                    ...
                    <div class="review-header">
                      ...
                      <template v-if="isMcReviewClient(review)">
                        ...
                        <div class="client-info">
                          ...
                          <div
                            class="review-date"
                            v-format-date="review.createdAt"
                          ></div>
                        </div>
                      </template>
                      <template v-else>
                  ...
                  <template #content>
                    <div
                      class="review-body"
                      :class="{ collapsed: review.collapsed }"
                    >
                      <Rating v-model="review.overallPoint" readonly></Rating>
                      <div class="event-name">
                        {{ review.contract?.eventName }}
                      </div>
                      ...
                      <template v-if="isMcReviewClient(review)">
                        ...
                        >
                          ...
                          <Rating
                            v-model="review.paymentPunctualPoint"
                            readonly
                          ></Rating>
                        </div>
                      </template>
                      ...
                      <template v-else>
                        <div
                          class="pro-point-wrapper flex align-center justify-between"
                        >
                        ...
                        <div
                          class="attitude-point-wrapper flex align-center justify-between"
                        >
                          <div class="label">Tinh thần thái độ</div>
                          <Rating
                            v-model="review.attitudePoint"
                            readonly
                          ></Rating>
                        </div>
                      </template>
                      ...
                      <div
                        class="reliable-point-wrapper flex align-center justify-between"
                      >
                        <div class="label">Độ tin cậy</div>
                        <Rating
                          v-model="review.reliablePoint"
                          readonly
                        ></Rating>
                      </div>
...
const hasEditPermission = computed(
  () => authStore.user!! && authStore.user.id == userId
);
...
const mcFormResolver = zodResolver(
  z.object({
    ...
    provinces: z
      .array(z.any())
      .min(1, { message: "Cần chọn ít nhất 1 giá trị" }),
    hostingStyles: z
      .array(z.any())
      .min(1, { message: "Cần chọn ít nhất 1 giá trị" }),
    facebook: z
      .string()
      .max(255, { message: "Không được vượt quá 255 ký tự" })
      .optional(),
    zalo: z
      .string()
      .max(255, { message: "Không được vượt quá 255 ký tự" })
...
const hostingStylesText = computed(() => {
  return (
    user.value.hostingStyles
      ?.map((style: HostingStyle) => style.label)
      .join(", ") || ""
  );
});
...
const areasText = computed(() => {
  return (
    user.value.provinces
      ?.map((province: Province) => province.name)
      .join(", ") || ""
  );
});
...
const handleSaveGeneralInfo = async (userSave: User) => {
  ...
  if (authStore.user?.isMc == "true") {
    userSave.mcTypes = updateEntityState(
      userSave.mcTypes,
      formInitialValues.value.mcTypes
    );
    userSave.hostingStyles = updateEntityState(
      userSave.hostingStyles,
      formInitialValues.value.hostingStyles
    );
    userSave.provinces = updateEntityState(
      userSave.provinces,
      formInitialValues.value.provinces
    );
  }
...
const handleDragEnd = async () => {
  ...
  toast.add({
    severity: "success",
    summary: "Images reordered successfully.",
    life: 3000,
  });
};
...
const fetchImages = async () => {
  const imagesFromApi = await mediaApi.getMediasByUserId(
    userId,
    MediaType.Image
  );
...
const onAddImageClick = () => {
  ...
  input.onchange = async (event: Event) => {
    ...
    if (target.files && target.files.length > 0) {
      ...
      const updatedMedias = await mediaApi.getMediasByUserId(
        userId,
        MediaType.Image
      );
...
const deleteVideo = async (index: number) => {
  ...
  toast.add({
    severity: "success",
    summary: "Video deleted successfully.",
    life: 3000,
  });
};
...
const handleVideoDragEnd = async () => {
  ...
  toast.add({
    severity: "success",
    summary: "Videos reordered successfully.",
    life: 3000,
  });
};
...
const fetchVideos = async () => {
  const videosFromApi = await mediaApi.getMediasByUserId(
    userId,
    MediaType.Video
  );
...
const fetchReviews = async () => {
  ...
  if (user.value.isMc) {
    reviews.value = reviews.value.concat(
      response.items.map((item: ClientReviewMc) => ({
        ...item,
        collapsed: true,
      }))
    );
    ...
    reviews.value = reviews.value.concat(
      response.items.map((item: McReviewClient) => ({
        ...item,
        collapsed: true,
      }))
    );
