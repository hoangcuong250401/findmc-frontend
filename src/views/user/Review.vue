    ...
    <div class="event-info-container">
      ...
      <div v-else class="info-item items-center">
        ...
        <div class="value">
          ...
          <span class="font-medium p-2">{{
            contract?.mc?.nickName || contract?.mc?.fullName
          }}</span>
        </div>
      </div>
      <div
        class="underline cursor-pointer view-contract-button"
        @click="viewContract"
      >
        Xem hợp đồng
      </div>
      ...
      >
        <div class="form-body flex flex-column gap-4">
          ...
          >
            ...
            <InputText
              name="shortDescription"
              v-model="review.shortDescription"
            />
            <Message
              v-if="$field?.invalid"
              severity="error"
          ...
          <template v-if="!isMc">
            ...
            >
              <label for="proPoint" class="form-label"
                >Điểm chuyên nghiệp</label
              >
          ...
          <template v-else>
            <FormField
              v-slot="$field"
              name="paymentPunctualPoint"
              class="flex flex-col gap-1 mt-3"
            >
              <label for="paymentPunctualPoint" class="form-label"
                >Điểm thanh toán đúng hạn</label
              >
              <Rating
                name="paymentPunctualPoint"
          ...
          <FormField
            ...
            class="flex flex-col gap-1"
          >
            <label for="detailDescription" class="form-label"
              >Mô tả chi tiết</label
            >
            <TextArea
              name="detailDescription"
              v-model="review.detailDescription"
            />
            <Message
              v-if="$field?.invalid"
              severity="error"
